<?php  
	...
	if (isset($_POST['simpan'])) {
		...
		$hapus = mysql_query("DELETE FROM `peserta_didik` WHERE `peserta_didik`.`no_pendaftar` = '$no_pendaftar'",$conn);
		...
		if ($data==true) {
            ...
            $id_pesdik3 = $id_pesdik2-1;
            $hapus2 = mysql_query("DELETE FROM `orang_tua` WHERE `orang_tua`.`id_pesdik` = '$id_pesdik3'",$conn);
            $hapus3 = mysql_query("DELETE FROM `wali_pesdik` WHERE `wali_pesdik`.`id_pesdik` = '$id_pesdik3'",$conn);
